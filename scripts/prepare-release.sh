...
release_file_marker="${promql_builder_path}/.release/tag"
...
release_branch_exists=$(git_has_branch "${promql_builder_path}" "${release_branch}")
if [ "$release_branch_exists" != "0" ]; then
  ...
  git_run "${promql_builder_path}" fetch origin "${release_branch}"
  git_run "${promql_builder_path}" checkout "${release_branch}"
  next_tag=$(cat "${release_file_marker}")
  git_run "${promql_builder_path}" checkout -

  debug "Deleting the local release branch"
  git_run "${promql_builder_path}" branch -D "${release_branch}"
fi

debug "Next version: ${next_tag}"

info "Creating new release branch"
git_run "${promql_builder_path}" checkout -b "${release_branch}"
...
info "Running cog"
run_codegen "output_dir=${codegen_output_path}/%l,package_version=${next_tag}"
...
if [ "${SKIP_VALIDATION}" == "yes" ]; then
  ...
  "$__dir/release-validate.sh" "${promql_builder_path}"
fi
...
git_run "${promql_builder_path}" commit -m "Prepare ${next_tag} release"

info "Pushing release branch ${release_branch}"
run_when_safe git_run "${promql_builder_path}" push origin "+${release_branch}"

debug "Ensuring that ${PROMQL_BUILDER_REPO} will be used by gh"
run_when_safe gh_run "${promql_builder_path}" repo set-default "${PROMQL_BUILDER_REPO}"

if [ "$release_branch_exists" != "0" ]; then
  info "Opening release Pull Request"
  run_when_safe gh_run "${promql_builder_path}" pr create \
    --base main \
    --head "${release_branch}" \
    --title "Next release" \
    --body "Note to maintainers: merging this PR will trigger the creation of a new release with all the modifications included on this branch. See the [release docs](https://github.com/grafana/promql-builder/blob/main/contributing/releasing.md) for more information."
fi

if [ "${DRY_RUN}" != "no" ]; then
  notice "Review the changes on the ${release_branch} branch in ${promql_builder_path} and re-run this script with DRY_RUN=no to disable dry-run mode."
